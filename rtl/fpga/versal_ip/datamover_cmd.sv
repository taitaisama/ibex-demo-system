...
module datamover_cmd #(
    ...
    parameter logic [31:0] dummy_base_addr = 32'h30100000
)(
    input wire                          clk,
    input wire                          rstn,
    ...
    output logic [LOG2_BUFFER_SIZE-1:0] hw_idx,
   ...
   logic [LOG2_BUFFER_SIZE-1:0]   next_cmd_send_idx;
   logic [LOG2_BUFFER_SIZE-1:0]   cmd_send_idx;

   logic [LOG2_BUFFER_SIZE-1:0]   next_hw_idx;

   always_comb begin

      // hw_idx always logically ahead of sw_idx
      // ncsi always logically ahead of hw_idx
      // don't want ncsi to overtake sw_idx

      // ncsi == sw_idx won't allow so
      // hw_idx == sw_idx shouldn't happen
      // it can happen at the start?
      
      // hw_idx < sw_idx <   ncsi  |    bad
      // hw_idx <   ncsi < sw_idx  |    good
      // sw_idx < hw_idx <   ncsi  |    good
      // sw_idx <   ncsi < hw_idx  |    bad
      //   ncsi < sw_idx < hw_idx  |    good
      //   ncsi < hw_idx < sw_idx  |    bad

      write_space_left = (next_cmd_send_idx != sw_idx) &&
                         (hw_idx < next_cmd_send_idx && next_cmd_send_idx < sw_idx) ||
                         (sw_idx <= hw_idx && hw_idx < next_cmd_send_idx) ||
                         (next_cmd_send_idx < sw_idx && sw_idx <= hw_idx);
      ...
      ADDR = cmd_send_idx + dummy_base_addr;
   ...
   always @(posedge clk or negedge rstn) begin
      if (!rstn) begin
         ...
         if (trigger && !m_axis_cmd_tvalid) begin
            // format: {RSVD:4, TAG:4, ADDR:32, DRR:1, EOF:1, DSA:6, TYPE:1, BTT:23}
