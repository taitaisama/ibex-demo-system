...
(
  ...
  output logic [37:0]			    debug,

  output logic [NUM_ID_BITS-1:0]	    info_rid,
  output logic [$clog2(READ_BURST_LEN)-1:0] info_rburst,
  ...
  input logic [NUM_ID_BITS-1:0]		    m_rid
   ...
   if (READ_BURST_LEN == 1) begin : gen_no_burst
	 ...
	 if (!rstn) begin
	    ...
	    read_fifo_valid <= fifo_data_valid;
	 end
      end
	 ...
	 this_read_counter = last_read_counter + 1;
      ...
      // fallthrough mode
      fifo_wrapper #(.WIDTH (NUM_ID_BITS), .DEPTH (128))
      u_pending_reads (
	 .clk (clk),
	 .rst (~rstn),
	 ...
	 if (last_read_valid && (last_read_burst_idx != READ_BURST_LEN-1) && (s_addr - last_read_addr) == 4) begin
      ...
      // size > 2**NUM_ID_BITS * READ_BURST_LEN
      // fallthrough mode
      fifo_wrapper #(.WIDTH (NUM_ID_BITS+$clog2(READ_BURST_LEN)), .DEPTH (128))
      u_pending_reads (
	 .clk (clk),
	 .rst (~rstn),
	 .data_valid (fifo_data_valid),
	 ...
	 m_rready = 1;
      ...
      assign debug = {s_gnt , s_req, this_read_in_burst, last_read_valid, last_read_burst_idx, last_read_addr};
   end
